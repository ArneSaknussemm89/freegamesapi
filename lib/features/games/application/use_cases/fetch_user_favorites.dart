...
final fetchUserFavoritesUseCaseProvider = Provider.autoDispose<FetchUserFavoritesUseCase>(
  (ref) {
...
class FetchUserFavoritesUseCase
  ...
  Future<UseCaseResult<Object?, List<FavoriteGame>>> execute(
    ...
    try {
      final favorites = await firestoreService.favorites.where('ownerId', isEqualTo: params.uid).get();
