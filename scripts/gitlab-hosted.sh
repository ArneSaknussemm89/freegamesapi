...
# This is called if the REPO_TYPE is "gitlab-hosted".
...
if [ "$CM_PULL_REQUEST" == "true" ]; then
    ...
    curl -H "Accept: application/json" -H "PRIVATE-TOKEN: $PRIVATE_TOKEN" "https://gitlab.codekoalas.com:443/api/v4/projects/$GITLAB_PROJECT_REPO/merge_requests/$CM_PULL_REQUEST_NUMBER" > mr.json
    ...
    UPDATED='{"description":'
    UPDATED+="\"$DESCRIPTION\"}"
    curl -H "Accept: application/json" -H "PRIVATE-TOKEN: $PRIVATE_TOKEN" -X PUT "https://gitlab.codekoalas.com:443/api/v4/projects/$GITLAB_PROJECT_REPO/merge_requests/$CM_PULL_REQUEST_NUMBER" -d $UPDATED
fi
